...
lazy_load_segment(struct page *page, void *aux)
{
	...
	// project 3 //////////////////////////////
	struct aux_box *auxx = (struct aux_box *)aux;

	off_t ofs = auxx->offs;
	uint8_t *upage = auxx->upage;
	uint32_t read_bytes = auxx->read_bytes;
	uint32_t zero_bytes = auxx->zero_bytes;
			 ...
			 uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
	...
	while (read_bytes > 0 || zero_bytes > 0)
	{
		...
		auxx->offs = ofs;
		auxx->upage = upage;
		auxx->read_bytes = page_read_bytes;
